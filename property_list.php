    ...
    if( $city_result && mysqli_num_rows($city_result)==1 ) //When $result1 is not false, and it contains single row.
    {
      ...
      if( isset($_GET["filter"]) )
      {
        if( $_GET["filter"]=="rent_desc" )
        {
          ...
          $sql_query2 = "SELECT id, name, address, gender, rent, rating_clean, rating_food, rating_safety, image FROM properties WHERE city_id='$city_id' ORDER BY rent DESC;";
...
//              $first_image = isset($images[0]) ? $images[0] : 'img/default.jpg'; // Use the first image or a default image
              ...
              $image_path = $property_row["image"]; // Fetch the image path from the database
              $default_image = 'img/properties/pglist16.png';  // Fallback image if no path is specified
              $image_to_display = !empty($image_path) ? $image_path : $default_image;
        ...
        ?>
              ...
              <div class="property-card row">
                 ...
                 <div class="image-container col-md-4">
    <img src="<?php echo $image_to_display; ?>" alt="Property Image" />
</div>
